...
Allow only a specific source IP or CIDR to access port `80` of the container `httpd`

    ufw-docker allow httpd 80/tcp --source 1.2.3.4
    ufw-docker allow httpd 80/tcp --source 192.168.1.0/24

The `--source` flag (also `--src` or `-s`) can be placed anywhere in the command. Without it, access is allowed from any source IP.
...
仅允许特定来源 IP 或 CIDR 访问容器 `httpd` 的 `80` 端口

    ufw-docker allow httpd 80/tcp --source 1.2.3.4
    ufw-docker allow httpd 80/tcp --source 192.168.1.0/24

`--source` 参数（也可以使用 `--src` 或 `-s`）可以放在命令的任意位置。不指定时，默认允许所有来源 IP 访问。
